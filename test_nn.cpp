...
static void namespace_test() {
  auto t0 = util::nn(new int(111));
  auto t1 = util::nn(new int(222));
  (void)t0;
  (void)t1;
}
...
int main() {
  ...
  auto t = nn{new int(7)};
  *t = 42;
  auto t2 = nn<pt*>{new pt(123, 123)};
  t2->x = 1;
  delete static_cast<int*>(t);
  delete static_cast<pt*>(t2);
  delete static_cast<int*>(nn<int*>(new int(7)));
  ...
  *c2 = pt{10, 10};
  ...
  int* x3;
  {
    /* Work around a Clang bug that causes an ambiguous conversion error
    here.
