...
template <typename T> struct element_type<std::weak_ptr<T>>
{
    using type = T;
};
} // namespace nn_detail
...
template <typename PtrType> class nn
{
  public:
    ...
    const PtrType &as_nullable() const &
    {
        return ptr;
    }
    PtrType &&as_nullable() &&
    ...
    explicit nn(i_promise_i_checked_for_null_t, PtrType &&arg) : ptr(std::move(arg))
    {
        assert(ptr);
    }
    ...
    template <typename OtherType, typename std::enable_if<std::is_constructible<PtrType, OtherType>::value &&
    ...
    template <typename OtherType,
              typename std::enable_if<std::is_convertible<OtherType, PtrType>::value, int>::type = 0>
    ...
    template <typename OtherType,
              typename std::enable_if<
                  std::is_convertible<OtherType, PtrType>::value && !std::is_pointer<OtherType>::value, int>::type = 0>
    ...
    template <typename OtherType,
              typename std::enable_if<std::is_constructible<PtrType, OtherType, element_type *>::value, int>::type = 0>
    ...
    template <typename L, typename R> friend bool operator==(const nn<L> &, const R &);
    template <typename L, typename R> friend bool operator==(const L &, const nn<R> &);
    template <typename L, typename R> friend bool operator==(const nn<L> &, const nn<R> &);
    ...
    // ostream operator
    template <typename T> friend std::ostream &operator<<(std::ostream &, const nn<T> &);
...
template <typename L, typename R> bool operator==(const nn<L> &l, const nn<R> &r)
{
    return l.ptr == r.ptr;
}
template <typename L, typename R> bool operator<(const nn<L> &l, const R &r)
...
template <typename T> std::ostream &operator<<(std::ostream &os, const nn<T> &p)
{
    return os << p.ptr;
}

#define NN_DERIVED_OPERATORS(op, base)                                                                                 \
    template <typename L, typename R> bool operator op(const nn<L> &l, const R &r)                                     \
    {                                                                                                                  \
        return base;                                                                                                   \
    }                                                                                                                  \
    template <typename L, typename R> bool operator op(const L &l, const nn<R> &r)                                     \
    {                                                                                                                  \
        return base;                                                                                                   \
    }                                                                                                                  \
    template <typename L, typename R> bool operator op(const nn<L> &l, const nn<R> &r)                                 \
    {                                                                                                                  \
        return base;                                                                                                   \
    }
...
template <typename T> using nn_weak_ptr = nn<std::weak_ptr<T>>;

template <typename T, typename... Args> nn_unique_ptr<T> nn_make_unique(Args &&...args)
{
    return nn_unique_ptr<T>(i_promise_i_checked_for_null, std::unique_ptr<T>(new T(std::forward<Args>(args)...)));
}

template <typename T, typename... Args> nn_shared_ptr<T> nn_make_shared(Args &&...args)
{
    return nn_shared_ptr<T>(i_promise_i_checked_for_null, std::make_shared<T>(std::forward<Args>(args)...));
}

template <typename T> class nn_enable_shared_from_this : public std::enable_shared_from_this<T>
{
  public:
...
template <typename T, typename U> nn_shared_ptr<T> nn_static_pointer_cast(const nn_shared_ptr<U> &org_ptr)
{
...
template <typename T, typename U> std::shared_ptr<T> nn_dynamic_pointer_cast(const nn_shared_ptr<U> &org_ptr)
{
...
template <typename T, typename U> nn_shared_ptr<T> nn_const_pointer_cast(const nn_shared_ptr<U> &org_ptr)
{
...
template <typename T> nn_weak_ptr<T> nn_make_weak(const nn_shared_ptr<T> &shared)
{
    return nn_weak_ptr<T>(i_promise_i_checked_for_null, std::weak_ptr<T>(shared.as_nullable()));
}

template <typename T> nn_shared_ptr<T> nn_lock_throw(const nn_weak_ptr<T> &weak)
{
    auto locked = weak.as_nullable().lock();
    if (!locked)
        throw std::runtime_error("weak_ptr expired");
    return nn_shared_ptr<T>(i_promise_i_checked_for_null, std::move(locked));
}

template <typename T> nn_shared_ptr<T> nn_lock(const nn_weak_ptr<T> &weak)
{
    auto locked = weak.as_nullable().lock();
    assert(locked && "weak_ptr expired");
    return nn_shared_ptr<T>(i_promise_i_checked_for_null, std::move(locked));
}

template <typename T> std::shared_ptr<T> nn_lock_safe(const nn_weak_ptr<T> &weak)
{
    return weak.as_nullable().lock();
}
...
#define NN_CHECK_ASSERT(_e)                                                                                            \
    (([&](typename std::remove_reference<decltype(_e)>::type p) {                                                      \
        ...
        if (!p)                                                                                                        \
            std::abort();                                                                                              \
        return util::nn<typename std::remove_reference<decltype(p)>::type>(util::i_promise_i_checked_for_null,         \
                                                                           std::move(p));                              \
    })(_e))
...
namespace std
{
template <typename T> struct hash<::util::nn<T>>
{
